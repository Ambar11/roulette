...
exports.makeTransaction = async(req, res) => {
    ...
    try {
        ...
        if (req.query.status == 'increment') {
            ...
            await functions.querySingle(`INSERT INTO transaction (refrence,cashier_id,type,points) VALUES (${userArray[0].id},${req.session.u_id},'CASHIER',${parseInt(amount)})`);

        } else if (req.query.status == 'decrement') {

            let totalPoints = parseInt(uPoints[0].points) - parseInt(amount);
            query = `UPDATE points SET points = ${totalPoints} WHERE id=${uPoints[0].id}`;
            await functions.querySingle(`INSERT INTO transaction (refrence,cashier_id,type,points,status) VALUES (${userArray[0].id},${req.session.u_id},'CASHIER',${parseInt(amount)},1)`);
